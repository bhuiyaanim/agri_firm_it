<?php
	error_reporting(0);
	
	if (isset($_POST['submit']))
	{
	
			$fname=trim($_POST['fname']);
			$lname=trim($_POST['lname']);
			$phone=trim($_POST['phone']);
			$email=trim($_POST['email']);
			$password=trim($_POST['password']);
			$cpassword=trim($_POST['cpassword']);
			$gender=$_POST['gender'];
			$type=$_POST['type'];
			$nid=$_POST['nid'];
			
			$fnameflag ="";
			$lnameflag ="";
			$emailflag ="";
			$phoneflag ="";
			$passwordflag ="";
			$cpasswordflag ="";
			$genderflag ="";
			$typeflag ="";
			
			
			if($fname =="" ||$phone == "" || $email == "" || $password == "" || $cpassword == "" || $gender == "" || $type == "" || $nid == "")
			{
				header("location:reg.php?Null not preferable");
			}
			else
			
			{
				if (ctype_alpha($fname)) 
					{
					$fnameflag ="1";
					}

				if (ctype_alpha($lname)) 
					{
					$lnameflag ="1";
					}	


				if( strlen($phone) >= 11)
				{
					$phoneflag = "1";
				}
				

				$pics_email = explode("@", $email); //Email split
				if($pics_email[0]!="" && $pics_email[1]!="")
				{
					$pics_email2 = explode(".", ($pics_email[1]));	
					if ((strlen($pics_email2[0])< 4)  && (strlen($pics_email2[1])> 2))	
					{
						$emailflag ="1";
					}
				}
				
				if (strlen($password > 5)  )
				{
					if(!ctype_alpha($password))
					{
					  $passwordflag="1";
			        }
				}
				
				if ($cpassword == $password)
				{
					$cpasswordflag="1";
				}
				
				
				$genderflag = "1";
				$typeflag = "1";
				
				if($fnameflag =1 && $lnameflag =1 && $emailflag =1 && $phoneflag =1 && $passwordflag =1 && $cpasswordflag =1 && $genderflag =1 &&	$typeflag =1)
				{
					

					$conn = mysqli_connect('', 'root', '', 'agrofirmit');

						if(!$conn){
							echo "DB Error: ".mysqli_connect_error();
						}
						else
						{
							
							
							$sql = "INSERT INTO `userreg`(`id`, `firstname`, `lastname`, `phone`, `email`, `password`, `gender`, `type`, `NID` ) VALUES ('','$fname','$lname','$phone','$email','$password','$gender','$type', '$nid')";
							$result = mysqli_query($conn, $sql);
							
							echo "DONE";
							header("location: ../login/Login.php?Status=registration_confirmed");
						
							mysqli_close($conn);
						
										
						}

					
					
				}
				else
				{
					header("location: registration.php?status=All cretientials are not ok!!");
				}
				
				
			} //else ending
			
	} //isset ending
	
	else
	{
		echo "Invalid request";
	}
	
?>